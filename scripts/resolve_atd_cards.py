...
def summarize(rows):
    ...
    out = {}
    for r in rows:
        blk = out.setdefault(r["mode"], {}).setdefault(str(r["n_legs"]),
...
def grade_records(records, by_week, finals, atd):
    rows, pending = [], 0
    for rec in records:
        week = int(rec["week"])
        for card in rec.get("cards") or []:
            ...
            if g["result"] == "pending":
                pending += 1
                continue
            rows.append({"week": week, "card_id": card["card_id"], "mode": card["mode"],
...
def run(cache_dir=None, out_path=OUT_PATH, now=None, offline=False):
    ...
    records = []
    for f in sorted(glob.glob(RECORD_GLOB)):
        try:
    ...
    if not records:
        ...
        rows, pending = grade_records(records, by_week, finals, atd)
