...
> **0%** (0/19 automatable). **14** name a test file that does not
> exist; **5** name a file that exists but contains no assertion that bites. The absent files are `tests/feature/player_projection.test.mjs`, `tests/feature/aging_curve.test.mjs`, `tests/feature/ol_dl_signal.test.mjs`, `tests/feature/targets_signal.test.mjs` and 1 more. Stories with **nothing asserted at all**: N2-S1, N2-S2, N2-S3, N2-S4, N2-S5, N2-S6.
...
### N2-S1 — Projection identity: baseline × signals → point + interval   ·  Status: 🟡   ·  Est: L   ·  **QA: 0/4 ACs asserted (0%)**
...
**QA coverage:**
- N2-S1-AC1 → `tests/feature/player_projection.test.mjs::day_zero_neutral` (unit) — Planned  **[MISSING]**
- N2-S1-AC2 → `tests/feature/player_projection.test.mjs::weight_gates_influence` (unit) — Planned  **[MISSING]**
- N2-S1-AC3 → `scripts/validate_data.py::player_projection_contract` (data) — Planned  **[TOOTHLESS · unwritten-case]**
- N2-S1-AC4 → `tests/feature/player_projection.test.mjs::band_labelled_estimate` (unit) — Planned  **[MISSING]**
...
### N2-S2 — Position-specific age curves   ·  Status: 🟡   ·  Est: M   ·  **QA: 0/3 ACs asserted (0%)**
...
**QA coverage:**
- N2-S2-AC1 → `tests/feature/aging_curve.test.mjs::plateau_is_one` (unit) — Planned  **[MISSING]**
- N2-S2-AC2 → `tests/feature/aging_curve.test.mjs::rb_cliff_below_qb` (unit) — Planned  **[MISSING]**
- N2-S2-AC3 → `tests/feature/aging_curve.test.mjs::continuous_monotone` (unit) — Planned  **[MISSING]**
...
### N2-S3 — OL-composite-vs-DL signal (mass + continuity)   ·  Status: 🟡   ·  Est: M   ·  **QA: 0/3 ACs asserted (0%)**
...
**QA coverage:**
- N2-S3-AC1 → `tests/feature/ol_dl_signal.test.mjs::matchup_direction` (unit) — Planned  **[MISSING]**
- N2-S3-AC2 → `tests/feature/ol_dl_signal.test.mjs::continuity_rewarded` (unit) — Planned  **[MISSING]**
- N2-S3-AC3 → `tests/feature/ol_dl_signal.test.mjs::rb_share_largest` (unit) — Planned  **[MISSING]**
- **Coverage (measured 2026-08-15): 0% — REAL 0/3 · MISSING 3.** Method + full matrix: [`../QA_COVERAGE.md`](../QA_COVERAGE.md).
**Traceability:** `scripts/signals/ol_dl.py`, `scripts/models/player_projection.py`.

### N2-S4 — Target / touch competition signal   ·  Status: 🟡   ·  Est: M   ·  **QA: 0/3 ACs asserted (0%)**
...
**QA coverage:**
- N2-S4-AC1 → `tests/feature/targets_signal.test.mjs::competitor_deflates` (unit) — Planned  **[MISSING]**
- N2-S4-AC2 → `tests/feature/targets_signal.test.mjs::departure_inflates` (unit) — Planned  **[MISSING]**
- N2-S4-AC3 → `tests/feature/targets_signal.test.mjs::shares_conserve` (unit) — Planned  **[MISSING]**
- **Coverage (measured 2026-08-15): 0% — REAL 0/3 · MISSING 3.** Method + full matrix: [`../QA_COVERAGE.md`](../QA_COVERAGE.md).
**Traceability:** `scripts/signals/targets.py`, `scripts/models/player_projection.py`.

### N2-S5 — Context signal suite (registered at weight 0)   ·  Status: 🟡   ·  Est: L   ·  **QA: 0/3 ACs asserted (0%)**
...
**QA coverage:**
- N2-S5-AC1 → `tests/feature/signal_registry.test.mjs::signals_enter_at_zero` (unit) — Planned  **[TOOTHLESS · unwritten-case]**
- N2-S5-AC2 → `tests/feature/weather_roof.test.mjs::indoor_is_neutral` (unit) — Planned  **[MISSING]**
- N2-S5-AC3 → `tests/feature/player_projection.test.mjs::day_zero_neutral` (unit) — Planned  **[MISSING]**
...
### N2-S6 — Evaluation: MAE + rank correlation vs actuals   ·  Status: 🟡   ·  Est: M   ·  **QA: 0/3 ACs asserted (0%)**
