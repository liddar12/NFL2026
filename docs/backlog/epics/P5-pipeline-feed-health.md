...
> Of this epic's **20 acceptance criteria**, **2 are asserted by a test that
> exists, runs in the gate, and fails when the criterion is violated** — a true coverage of
> **11%** (2/19 automatable). **14** name a test file that does not
> exist; **3** name a file that exists but contains no assertion that bites. **1** are manual/ops ACs, excluded from the denominator. The absent files are `tests/feature/pipeline_status.test.mjs`, `tests/feature/pipeline_scrape.test.mjs`, `tests/feature/frozen_guard.test.mjs`, `tests/feature/determinism.test.mjs` and 2 more. Stories with **nothing asserted at all**: P5-S2, P5-S4, P5-S5, P5-S6.
...
### P5-S1 — Loud row-count + staleness assertions on every scraper   ·  Status: 🟡   ·  Est: M   ·  **QA: 1/4 ACs asserted (25%)**
...
**QA coverage:**
- P5-S1-AC1 → `tests/feature/pipeline_scrape.test.mjs::short_pull_raises` (unit, shells to python) — Planned  **[MISSING]**
- P5-S1-AC2 → `tests/feature/pipeline_scrape.test.mjs::stale_pull_raises` (unit) — Planned  **[MISSING]**
- P5-S1-AC3 → `tests/feature/pipeline_scrape.test.mjs::no_date_raises` (unit) — Planned  **[MISSING]**
- P5-S1-AC4 → `tests/smoke.sh::imports` (smoke — module imports with no pip) — Done  **[REAL]**
...
### P5-S2 — No continue-on-error may mask a zero write   ·  Status: 🟡   ·  Est: S   ·  **QA: 0/2 ACs asserted (0%)**
...
**QA coverage:**
- P5-S2-AC1 → `tests/feature/workflow_guards.test.mjs::no_masking_true` (unit — parse yml, assert no masking on scraper steps) — Planned  **[MISSING]**
- P5-S2-AC2 → manual (Gate-2 wiring review) — Planned (counts toward story: 2 of 3 ACs automatable)  **[MANUAL]**
- P5-S2-AC3 → `tests/feature/pipeline_status.test.mjs::down_exits_2` (unit, shells to python) — Planned  **[MISSING]**
  - **Coverage (measured 2026-08-15): 0% — REAL 0/2 · MISSING 2 · manual 1 (excluded).** Method + full matrix: [`../QA_COVERAGE.md`](../QA_COVERAGE.md).
...
### P5-S3 — `pipeline_status.json` tracks per-feed rows/age/last_success/status   ·  Status: 🟡   ·  Est: M   ·  **QA: 1/4 ACs asserted (25%)**
...
**QA coverage:**
- P5-S3-AC1 → `tests/feature/pipeline_status.test.mjs::decision_order` (unit, pinned as_of) — Planned  **[MISSING]**
- P5-S3-AC2 → `tests/feature/pipeline_status.test.mjs::zero_rows_is_down` (unit) — Planned  **[MISSING]**
- P5-S3-AC3 → `tests/feature/pipeline_status.test.mjs::empty_monitor_all_down` (unit) — Planned  **[MISSING]**
- P5-S3-AC4 → `scripts/validate_data.py` (data — schema check of pipeline_status.json) — Done  **[REAL]**
  - **Coverage (measured 2026-08-15): 25% — REAL 1/4 · MISSING 3.** Method + full matrix: [`../QA_COVERAGE.md`](../QA_COVERAGE.md).
...
**QA coverage:**
- P5-S5-AC1 → `tests/feature/frozen_guard.test.mjs::constant_signal_flagged` (unit) — Planned  **[MISSING]**
- P5-S5-AC2 → `tests/feature/frozen_guard.test.mjs::identical_payload_flagged` (unit) — Planned  **[MISSING]**
- P5-S5-AC3 → `tests/feature/frozen_guard.test.mjs::frozen_degrades_feed` (unit) — Planned  **[MISSING]**
